...
export function Skills() {
  ...
  function initTree(graphElement: HTMLDivElement) {
    ...
    const groupColors: string[] = ["#1d948f", "#1d941d", "#32ec32", "#ccc"];
    ...
    graph
      ...
      .onNodeHover(
        (node: any) =>
          (graphElement.style.cursor =
            node && node.children.length ? "pointer" : null)
      )
      ...
      .linkPositionUpdate((object3d, { start, end }) => {
        ...
        const positions = [startR / lineLen, 1 - endR / lineLen].map((t) =>
          ["x", "y", "z"].map(
            (dim) =>
              (start as any)[dim] +
    ...
    myGraph.d3Force("link").distance(() => 50);
