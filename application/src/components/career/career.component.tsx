...
export function Career() {
	const [
		selectedStation,
		setSelectedStation
	] = createSignal<CareerStation>(null);
	const [
		forceActive,
		setForceActive
	] = createSignal(0);
	...
	function updateForceActive(force?: boolean) {
		...
		document.querySelectorAll('.career__station')
			.forEach((station: HTMLElement, index: number) => {
									...
									<div class="career__station__title">
										<div class="career__station__date">
											{station.start.month.toString()
												.padStart(2, '0')} {station.start.year}&nbsp;
											<Show when={station.end.month != station.start.month || station.end.year !== station.start.year}>
												&mdash; {station.end.month.toString()
													.padStart(2, '0')} {station.end.year}
											</Show>
										</div>
										<div>{(index() + 1).toString()
											.padStart(stations.length.toString().length, '0')}. {station.job}</div>
										<div class="career__station__sub-title">
											@ <For each={station.companies}>
												{(item, index) =>
													<>
														<span>{typeof item === 'string'
															? item
															: (item as CompanyInfo).name}</span>
